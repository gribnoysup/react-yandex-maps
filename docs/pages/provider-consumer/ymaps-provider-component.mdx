...
const Application = () => (
  <YMaps query={{ lang: 'en_RU' }}>
    Application with YMaps Provider
  </YMaps>
);
...
> `INFO` Keep in mind that by default provider will fetch only the **critical part**
> of Yandex.Maps API. After that every map component will handle fetching corresponding
